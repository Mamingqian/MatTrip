...
def create_tourists():
    ...
    for i in range(3000):
        ...
        tmp.interest_in_concept["Amusement"] = r1
        tmp.interest_in_concept["Park"] = r2
        tmp.interest_in_concept["Historical"] = r3
        tmp.interest_in_concept["Entertainment"] = r4
        tourists[i] = tmp
    return tourists

def generate_tourist(interest):
    tourists = []
    tmp = Tourist(0, 0, 0)
    tmp.interest_in_concept["Amusement"] = float(interest[0])
    tmp.interest_in_concept["Park"] = float(interest[1])
    tmp.interest_in_concept["Historical"] = float(interest[2])
    tmp.interest_in_concept["Entertainment"] = float(interest[3])
    tourists.append(tmp)
    return tourists
...
def poi_2_group(pois, tourists, costs, start, end, improved=True,):
    ...
    for tourist in tourists:
        interest_in_concept = tourists[tourist].interest_in_concept
...
def write_path(tourists, paths):
    write_path = "D:\\Shanghai JiaoTong University\\Research\\tourist recommendation\\RNN\\data\\Osaka\\fake_path.txt"
...
def write_user(tourists):
    path = "D:\\Shanghai JiaoTong University\\Research\\tourist recommendation\\RNN\\data\\Osaka\\fake_user.txt"
    text_file = open(path, "w+")
    for tourist in tourists:
